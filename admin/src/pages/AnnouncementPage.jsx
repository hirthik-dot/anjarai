...
import { Plus, Trash2, Edit2, GripVertical, Save, X, Megaphone } from 'lucide-react';
...
export default function AnnouncementPage() {
  ...
  const handleReorder = async (newOrder) => {
    setItems(newOrder);
    try {
      await api.post('/announcements/reorder', { order: newOrder.map(s => s._id) });
    } catch (err) { toast.error('Reorder sync failed'); }
  };

  return (
    <div className="space-y-12 animate-in fade-in duration-500">
      <h2 className="font-head text-4xl font-black text-brand-dark tracking-tight">📢 Announcements</h2>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        {/* Editor */}
        <div className="lg:col-span-1">
          <SectionCard title={editing ? "Edit Text" : "Add Announcement"} subtitle="Scrolling bar at the very top">
            <form onSubmit={handleSave} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-brand-mid/50 ml-2">Announcement Text *</label>
                <textarea 
                  value={form.text} onChange={(e) => setForm(f => ({ ...f, text: e.target.value }))}
                  placeholder="e.g. 🚚 Free Shipping on Orders Above ₹500"
                  className="w-full bg-brand-light/50 border-2 border-brand-green-pale rounded-2xl px-4 py-3 text-sm font-bold focus:border-brand-green outline-none min-h-[120px]"
                />
              </div>
              ...
              <div className="flex gap-4">
                <button type="submit" className="flex-1 bg-brand-green text-white font-black rounded-2xl py-4 shadow-lg shadow-brand-green/20 uppercase tracking-widest flex items-center justify-center gap-2 transition-all hover:scale-105">
                  <Save size={18} /> {editing ? 'UPDATE TEXT' : 'ADD TO BAR'}
                </button>
                {editing && (
          ...
          {/* Preview Bubble */}
          <div className="mt-8 bg-brand-green p-4 rounded-3xl shadow-xl shadow-brand-green/10 flex items-center justify-center overflow-hidden relative">
            <div className="absolute inset-0 opacity-10 bg-[radial-gradient(#fff_1px,transparent_1px)] bg-[length:10px_10px]" />
            <motion.div 
               animate={{ x: [200, -200] }} transition={{ repeat: Infinity, duration: 10, ease: 'linear' }}
               className="text-white text-[10px] font-black uppercase tracking-widest whitespace-nowrap"
            >
              🚚 {form.text || 'YOUR ANNOUNCEMENT PREVIEW HERE...'} 🚚
            </motion.div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          <h3 className="text-xl font-black text-brand-dark mb-6 flex items-center gap-3">
             <Megaphone className="text-brand-green" />
             Active Bar List (Drag to Reorder)
          </h3>
            ...
            {items.map(item => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h4 className="text-sm font-bold text-brand-dark leading-relaxed line-clamp-2">{item.text}</h4>
                </div>
      ...
      <ConfirmDialog 
        ...
        message="This will remove the text from the top announcement bar."
